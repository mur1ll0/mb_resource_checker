...
call "C:\Qt\5.15.2\msvc2019_64\bin\windeployqt.exe" release\mb_resource_checker.exe
if %errorlevel% neq 0 (
    echo [ERROR] windeployqt failed.
    exit /b %errorlevel%
)

echo [BUILD] Packaging release folder into ZIP...
powershell -Command "Compress-Archive -Path release\* -DestinationPath mb_resource_checker.zip -Force"
if %errorlevel% neq 0 (
...
rc.exe /fo release\loader.res src\loader.rc
if %errorlevel% neq 0 (
...
cl.exe /O2 /MT /EHsc src\loader.cpp src\miniz.c release\loader.res /Fe"Motherboard Resource Checker.exe" /link /SUBSYSTEM:WINDOWS user32.lib shell32.lib
if %errorlevel% neq 0 (
